...
import Pagination from '@mui/material/Pagination'; // updated import
...
const TableComponent = ({
    ...
    const totalColumns = !!selectType
        ? showOrdinalNumbers
            ? columns?.filter((item) => !item?.isHide)?.length + 2
            : columns?.filter((item) => !item?.isHide)?.length + 1
        : showOrdinalNumbers
            ? columns?.filter((item) => !item?.isHide)?.length + 1
            : columns?.filter((item) => !item?.isHide)?.length;
                                    ...
                                    <tr
                                        ...
                                        className={clsx(
                                            `row-table-body pointer row-selection ${
                                                chooseRow && chooseRow?.id === row?.id ? 'row-choose' : ''
                                            }`,
                                            (selectedRow(row) ||
